...
- [전체 아키텍처](#전체-아키텍처)
- [페이지 구성](#페이지-구성)
- [페이지별 렌더링 전략](#페이지별-렌더링-전략)
- [레이아웃](#레이아웃)
- [로딩 UI](#로딩-ui)
- [비주얼 아이덴티티 (컬러 / 타이포그래피 / 아이콘)](#비주얼-아이덴티티-컬러--타이포그래피--아이콘)
- [확정 사항 (사용자 결정)](#확정-사항-사용자-결정)
- [기능 요구사항 (CRUD / 권한)](#기능-요구사항-crud--권한)
- [숨김 글 미리보기 (Draft Mode)](#숨김-글-미리보기-draft-mode)
- [로그인 모달](#로그인-모달)
- [인증 구현 (BFF, 라이브러리 없이 자체 구현)](#인증-구현-bff-라이브러리-없이-자체-구현)
- [댓글 시스템 (v1 스코프 제외)](#댓글-시스템-v1-스코프-제외)
- [글 추천(좋아요) (v1 스코프 제외)](#글-추천좋아요-v1-스코프-제외)
- [통합 검색](#통합-검색)
- [소개 페이지 (`/about`)](#소개-페이지-about)
- [데이터 모델 (프론트에서 바라보는 형태, 백엔드 API 응답 기준)](#데이터-모델-프론트에서-바라보는-형태-백엔드-api-응답-기준)
- [이미지 업로드 (글 작성 중 첨부)](#이미지-업로드-글-작성-중-첨부)
- [마크다운 작성 / 렌더링](#마크다운-작성--렌더링)
- [포트폴리오 강화 기능](#포트폴리오-강화-기능)
- [에러 처리](#에러-처리)
- [테스트](#테스트)
- [Storybook (미니 디자인 시스템)](#storybook-미니-디자인-시스템)
- [컴포넌트 / 파일 구조](#컴포넌트--파일-구조)
- [필요 shadcn 컴포넌트](#필요-shadcn-컴포넌트)
- [배포 / CI-CD](#배포--ci-cd)
- [향후 고도화 아이디어 (지금 스코프 아님, 돈이 드는 것부터 나중으로 보류)](#향후-고도화-아이디어-지금-스코프-아님-돈이-드는-것부터-나중으로-보류)
- [다음 단계 (단계별)](#다음-단계-단계별)
...
src/
  ...
  hooks/
    ...
    use-category-groups.ts           # 사이드바 카테고리 아코디언용 — GET /posts를 클라이언트에서 category→tags로 그룹핑
  # data/ 디렉터리(categories.ts, posts.ts 목업)는 28번에서 실 API 전환 후 삭제 — lib/posts.ts가 그 자리를 대신함
...
26-1. [x] `react-error-boundary` 설치 + `QueryErrorResetBoundary`와 연동, `use-stats.ts` 등 실제 API 훅에 위젯 단위 에러 바운더리 적용 ("에러 처리" 섹션 참고) — 29번에서 `stats-widget.tsx`를 실제로 붙일 때 함께 진행
...
29. [x] `use-stats.ts` + `stats-widget.tsx`를 실제 API로 전환 (목업 데이터 제거) — shadcn `chart`(Recharts 래퍼) + `card` 신규 설치, `react-error-boundary` 설치해 26-1번도 같이 처리. `usePopularPosts`/`useVisits`(TanStack Query, `throwOnError: true`로 실패 시 쿼리 에러 대신 실제로 throw해서 에러 바운더리가 잡게 함) 구현. `stats-widget.tsx`는 `QueryErrorResetBoundary` + `react-error-boundary`의 `<ErrorBoundary onReset={reset} fallbackRender={...}>`로 감싸 위젯 하나가 실패해도 홈페이지 나머지는 안 무너지게 함(재시도 버튼 포함) — 공식 문서가 제시하는 표준 패턴 그대로. 인기글 TOP5는 순위+제목+조회수 목록, 조회 추이는 `LineChart`(날짜 `M/D` 포맷). 홈(`page.tsx`)의 플레이스홀더를 교체
    - 실제 blog-api + 프로덕션 빌드로 검증: 조회수를 다르게 만든 글 2개로 인기글 순위가 실제로 조회수 순으로 나오는지, 오늘 방문 추이 그래프에 실제 조회 기록이 반영되는지 Playwright로 확인, 콘솔 에러 0개. 테스트로 만든 글/카테고리/오염된 오늘자 `DailyVisit`은 검증 후 정리
29-1. [x] **배포 전 코드 리뷰 반복 검증** — `/code-review`로 여러 각도(재사용성, 단순화/효율, 과설계, 컨벤션, 라인별 diff, 크로스 파일 추적, 제거된 동작 감사)의 병렬 서브에이전트를 돌려 실제 버그를 찾고 심각도 순으로 고침. 각 수정은 `tsc --noEmit` + `yarn lint` + `yarn build`(Node 20+ 필요 — 로컬 기본 Node 18로는 Next 16/Playwright가 안 돌아서 `nvm use 24`로 전환) + 실제 blog-api(Node 18에선 Prisma 7이 전역 `crypto`를 못 찾아 `--experimental-global-webcrypto` 필요) 로컬 기동 후 curl/Playwright로 재검증
    - **진짜 버그(고침)**: `sitemap.ts`/`rss.xml/route.ts`가 draft-mode 인식하는 `fetchPosts`를 쓰고 있어서, 소유자가 로그인(=draft mode 켜짐)한 채로 이 라우트가 렌더되면 숨김 글이 공개 sitemap/RSS에 노출될 수 있었음(+ RSS의 `revalidate=3600`도 무효화됨) → 둘 다 draft 인식이 아예 없는 `fetchPublicPosts`로 교체. 실제로 글 생성→숨김 처리 후 sitemap/rss에 안 뜨는 것과, `fetchPublicPosts`가 `cookies()`/`draftMode()`를 아예 호출하지 않아 구조적으로 숨김 글을 볼 수 없다는 것 둘 다 확인
    - **진짜 버그(고침)**: 조회수 증가 API(`POST /posts/[slug]/view`)가 백엔드/BFF 프록시까지 다 만들어져 있었는데 프론트 어디서도 호출하는 곳이 없어서 "인기글 TOP5" 통계가 사실상 죽어있었음 → `view-tracker.tsx`(마운트 시 `fetch(...POST)`) 신설, `/posts/[slug]` 상세 페이지에 배치. Playwright로 실제 브라우저에서 페이지 진입 시 POST 요청이 나가는 것과 백엔드 `viewCount`가 실제로 증가하는 것까지 확인
    - **진짜 버그(고침)**: `app/api/posts/[slug]/route.ts`와 `.../view/route.ts`가 슬러그를 `encodeURIComponent` 없이 그대로 백엔드 경로에 꽂고 있었음 — 슬러그 생성기(`blog-api`의 `slugify.ts`)가 한글을 그대로 보존하도록 설계돼 있어(로마자 변환 안 함, 의도된 설계) 한글 제목 글의 슬러그는 URL 인코딩이 필수인데 빠져있었던 것. 한글 슬러그 글로 재현 후 인코딩 추가해 고침
    - **진짜 버그(고침)**: `lib/api.ts`의 `proxyToBackend`가 에러 응답 바디 파싱 실패 시 `null`로 폴백하고 있어서, 클라이언트의 `postJson()`이 `error.message`에 접근할 때 `null.message`로 죽을 수 있었음 → `{}`로 폴백하도록 수정
    - **진짜 버그(고침)**: `post-form.tsx`가 `useState(initialValues.x)`로 폼을 초기화하는데 `key` prop이 없어서, 이미 TanStack Query 캐시에 있는 다른 글의 수정 페이지로 클라이언트 사이드 이동하면(예: A 글 수정 페이지 갔다가 뒤로 갔다 다시 B 글 수정 페이지로) 폼이 리마운트 안 되고 이전 글의 입력값이 새 글의 슬러그에 얹혀 저장될 위험이 있었음 → `posts/[slug]/edit/page.tsx`에서 `<PostForm key={post.slug} .../>` 추가
    - **진짜 버그(고침)**: `login-modal.tsx`/`header.tsx`의 로그인·로그아웃이 draft mode도 서버에서 같이 켜고 끄는데(23번 로그인/로그아웃 라우트 참고) `router.refresh()`가 없어서 이미 렌더된 서버 컴포넌트(숨김 글 미리보기 등)가 쿠키 상태 변화를 바로 반영하지 못했음 → 둘 다 `router.refresh()` 추가, 로그아웃은 `invalidateQueries`도 await하도록 수정(잠깐 `isOwner`가 stale하게 true로 남는 창 제거)
    - **방어적 정합성(고침)**: `use-categories.ts`/`use-tags.ts`/`use-category-groups.ts`/`use-posts.ts`(검색)가 `fetch(...).then(r => r.json())`만 하고 `response.ok`를 안 봐서, 백엔드 에러 응답(`{statusCode, message}`)을 그대로 데이터인 척 쓰다가 `.map is not a function` 같은 크래시나 "검색 결과 없음"으로 오인되는 조용한 오동작이 날 수 있었음 → 카테고리/태그/카테고리그룹은 보조 데이터라 `response.ok` 체크 후 빈 배열로 우아하게 성능 저하, 검색은 `/search` 페이지의 핵심 콘텐츠라 대신 throw해서 `search-content.tsx`에 별도 에러 문구 분기 추가
    - **자잘한 버그(고침)**: `stats-widget.tsx`의 `formatDate`가 `new Date("YYYY-MM-DD")`(UTC 자정)로 파싱한 뒤 `getMonth()/getDate()`(로컬 시간)로 읽어서 음수 UTC 오프셋 지역 방문자에게 하루 밀려 보일 수 있었음 → 문자열 직접 파싱으로 교체. `tag-input.tsx`의 대소문자 구분 중복 체크(대소문자만 다른 태그가 전역 태그 목록에 이미 있으면 대소문자 다른 채로 중복 추가 가능) → 이미 존재하는 `alreadyExists`의 대소문자 무시 비교 규칙과 통일. `owner-actions.tsx`의 숨김/고정/삭제 버튼에 뮤테이션 진행 중 `disabled` 누락(빠른 연타 시 중복 요청 가능) → 추가. `category-combobox.tsx`가 "로딩 중"과 "진짜 목록 없음"을 구분 안 해서 로딩 중에도 "카테고리가 없습니다" 문구가 잠깐 뜸 → `isLoading` 분기 추가
    - **검토했지만 안 고친 것**: `proxyToBackend`(클라이언트 프록시 라우트 전용, 쿠키에 JWT 있으면 draft mode 여부와 무관하게 항상 포함)와 `lib/posts.ts`의 `authHeaders()`(draft mode 켜졌을 때만 JWT 포함, SSR 페이지 전용)가 서로 다른 기준으로 동작 — 다만 두 경로의 설계 의도 자체가 다름(SSR 공개 페이지는 draft mode로 숨김 글 미리보기를 명시적으로 제어해야 하지만, 에디터 등 CSR 소유자 전용 화면은 로그인 여부만으로 판단하는 게 맞음), 로그인/로그아웃이 토큰 쿠키와 draft 쿠키를 항상 같이 세팅/해제하고 둘 다 세션 쿠키라 실사용에서 어긋날 창이 매우 좁고, 어긋나도 사이드바 태그 집계 개수 정도만 잠깐 부정확해질 뿐 실제 컨텐츠 노출 사고로는 안 이어짐 — 성급하게 통일시켰다가 에디터의 정당한 숨김 글 접근을 깨뜨릴 위험이 더 커서 보류. 재사용성 관련 지적(에러 메시지 파싱 로직 3곳 중복, 쿼리스트링 조립 2곳 중복 등)은 정확성 문제가 아니라 보류
